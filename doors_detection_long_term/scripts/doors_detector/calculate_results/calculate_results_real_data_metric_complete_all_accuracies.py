...
def compute_results(model, data_loader_test, confidence):
    model.eval()
    model.to(device)
    ...
    metrics = evaluator.get_metrics(iou_threshold=0.75, confidence_threshold=confidence, door_no_door_task=False, plot_curves=False)
    mAP = 0
...
for confidence in [0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95]:
    results = []
    # General detectors
    for dataset, epoch_general in [(d, eg) for d in general_datasets for eg in epochs_general_detector]:
        ...
        for house in houses:
            ...
            metrics = compute_results(model, data_loader_test, confidence)
    ...
    save_file(results, f'results_real_data_metric_complete_confidence_{confidence}.xlsx')