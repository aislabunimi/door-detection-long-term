...
from doors_detection_long_term.scripts.doors_detector.dataset_configurator import \
    ...
    get_final_doors_dataset_real_data_multiple_houses_different_test

houses = ['floor4','floor4_evening', 'floor4_run2']# 'chemistry_floor0']
...
def compute_results(model_name, data_loader_test):
    ...
    for iou_threshold in np.arange(0.5, 0.96, 0.05):
        for confidence_threshold in np.arange(0.5, 0.96, 0.05):
            ...
            metrics[(iou_threshold, confidence_threshold)] = evaluator.get_metrics(iou_threshold=iou_threshold, confidence_threshold=confidence_threshold, door_no_door_task=False, plot_curves=False, colors=[])
            complete_metrics[(iou_threshold, confidence_threshold)] = evaluator_complete_metric.get_metrics(iou_threshold=iou_threshold, confidence_threshold=confidence_threshold, door_no_door_task=False, plot_curves=False, colors=[])
...
# General detectors
for house, dataset, epochs_gd in [(h, d, e) for h in ['floor4_run2'] for d in datasets for e in epochs_general_detector]:
    ...
    train, validation, test, labels, _ = get_final_doors_dataset_real_data_multiple_houses_different_test(
        folder_names_train=train_envs, folder_names_test=[house], train_size=0.25
    )
    ...
    metrics, complete_metrics = compute_results(model_name, data_loader_test, )
...
for house, dataset, epochs_gd, epochs_qd, fine_tune in [(h, d, e, eq, ft) for h in ['floor4_run2'] for d in datasets for e in [60] for eq in epochs_qualified_detector for ft in  fine_tune_quantity]:
    train_envs = list(houses)
    train_envs.remove(house)
    train, validation, test, labels, _ = get_final_doors_dataset_real_data_multiple_houses_different_test(
        folder_names_train=train_envs, folder_names_test=[house], train_size=0.25
    ...
    data_loader_test = DataLoader(test, batch_size=1, collate_fn=collate_fn, drop_last=False, num_workers=4)
    model_name = globals()[f'EXP_2_{house}_{dataset}_{epochs_gd}_FINE_TUNE_{fine_tune}_EPOCHS_{epochs_qd}_DAY_NIGHT'.upper()]
