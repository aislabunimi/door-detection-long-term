...
from doors_detection_long_term.doors_detector.models.gibson_google import GibsonGoggle
...
def compute_results(model_name, data_loader_test, description, with_goggle):
    ...
    complete_metrics = {}
    metrics = {}
    for iou_threshold in [0.5, 0.75]:#np.arange(0.5, 0.96, 0.05):
        for confidence_threshold in [0.75]: # np.arange(0.5, 0.96, 0.05):
...
def save_file(results, complete_results, file_name_1, file_name_2):
    ...
    columns = ['goggle', 'iou_threshold', 'confidence_threshold', 'house', 'detector', 'dataset', 'epochs_gd', 'epochs_qd', 'label',  'AP', 'total_positives', 'TP', 'FP']
    d = {}
        ...
        for with_goggle in [0, 1]:
            ...
            for (iou_threshold, confidence_threshold), metric in metrics.items():
                for label, values in sorted(metric['per_bbox'].items(), key=lambda v: v[0]):
                    results += [[with_goggle, iou_threshold, confidence_threshold, house, 'GD', dataset, epochs, epochs, label, values['AP'], values['total_positives'], values['TP'], values['FP']]]
            ...
            for (iou_threshold, confidence_threshold), complete_metric in complete_metrics.items():
                for label, values in sorted(complete_metric.items(), key=lambda v: v[0]):
                    results_complete += [[with_goggle, iou_threshold, confidence_threshold, house.replace('_', ''), 'GD', dataset, epochs, epochs, label, values['total_positives'], values['TP'], values['FP'], values['TPm'], values['FPm'], values['FPiou']]]
...
"""
save_file(results, results_complete, 'yolov5_ap_real_data_goggle.xlsx', 'yolov5_complete_metric_real_data_goggle.xlsx')
