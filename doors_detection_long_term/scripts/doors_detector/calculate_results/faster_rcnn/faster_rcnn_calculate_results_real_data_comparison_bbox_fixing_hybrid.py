...
def compute_results(model_name, data_loader_test, description):
    model = FasterRCNN(model_name=FASTER_RCNN, n_labels=3, pretrained=True, dataset_name=FINAL_DOORS_DATASET, description=model_name, box_score_thresh=0.0, box_nms_thresh=1.0, box_detections_per_img=300)
    model.eval()
    model.to(device)
    ...
    with torch.no_grad():
        for images, targets, converted_boxes in tqdm(data_loader_test, total=len(data_loader_test), desc=description):
            ...
            for pred in preds:
                ...
                n=50
                """scores, indices = torch.sort(pred['scores'], descending=True)
                pred['scores'] = scores[:n]
                pred['labels'] = pred['labels'][indices][:n]
                pred['boxes'] = pred['boxes'][indices][:n]
                """
...
def save_file(results, complete_results, file_name_1, file_name_2):
    ...
    columns = ['iou_threshold', 'confidence_threshold', 'house', 'detector',  'label',  'AP', 'total_positives', 'TP', 'FP']
    d = {}
    ...
    columns = ['iou_threshold', 'confidence_threshold', 'house', 'detector', 'label',  'total_positives', 'TP', 'FP', 'TPm', 'FPm', 'FPiou']
    d = {}
    for i, column in enumerate(columns):
...
for house in houses:
    ...
    model_name = globals()[f'EXP_GENERAL_DETECTOR_HYBRID_{house}_60_EPOCHS'.upper()]
    metrics, complete_metrics = compute_results(model_name, data_loader_test, f'Test on {house}')
    ...
    for (iou_threshold, confidence_threshold), metric in metrics.items():
        for label, values in sorted(metric['per_bbox'].items(), key=lambda v: v[0]):
            results += [[iou_threshold, confidence_threshold, house, 'GD', label, values['AP'], values['total_positives'], values['TP'], values['FP']]]
    ...
    for (iou_threshold, confidence_threshold), complete_metric in complete_metrics.items():
        for label, values in sorted(complete_metric.items(), key=lambda v: v[0]):
            results_complete += [[iou_threshold, confidence_threshold, house.replace('_', ''), 'GD', label, values['total_positives'], values['TP'], values['FP'], values['TPm'], values['FPm'], values['FPiou']]]


save_file(results, results_complete, 'faster_rcnn_ap_real_data_comparison_hybrid.xlsx', 'faster_rcnn_complete_metric_real_data_comparison_hynrid.xlsx')