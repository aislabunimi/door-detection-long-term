...
houses = ['floor4', ]#'chemistry_floor0']
...
for model_name, dataset, epochs, in model_names_general_detectors:
    ...
    for house in houses:
        train, test, _, _ = get_final_doors_dataset_real_data(folder_name=house + '_crowded', train_size=0.75, transform_train=False)
...
for model_name, house, dataset, quantity, epochs_general, epochs_qualified in model_names_qualified_detectors:
    train, test, labels, COLORS = get_final_doors_dataset_real_data(folder_name=house + '_crowded', train_size=0.75)
    data_loader_train = DataLoader(train, batch_size=1, collate_fn=collate_fn_faster_rcnn, drop_last=False, num_workers=4)
...
save_file(results, results_complete, 'faster_rcnn_ap_real_data_crowded.xlsx', 'faster_rcnn_complete_metric_real_data_crowded.xlsx')