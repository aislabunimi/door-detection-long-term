...
from doors_detection_long_term.doors_detector.dataset.dataset_doors_final.door_sample import DoorSample as DoorSampleFinal
...
def next_img_index():
    ...
    #img_objects = sample.get_bounding_boxes()
...
while True:
    ...
    if pressed_key == ord('a') or pressed_key == ord('d'):
        ...
        # show next image key listener
            ...
            label = 0 if len(img_objects) == 0 else 1
            door_sample_final = DoorSampleFinal(label=label)
            door_sample_final.set_bgr_image(sample.get_bgr_image().copy())
            door_sample_final.set_bounding_boxes(value=np.array(img_objects, dtype=int))

            # Convert depth data to depth image
            ...
            door_sample_final.set_depth_image(pipeline_generate_depth_image.set_data(data=sample.get_depth_data()).set_end_function(lambda x: x).run(use_gpu=False).get_data())
            folder_manager_save.save_sample(door_sample_final, use_thread=True)
            next_img_index()
