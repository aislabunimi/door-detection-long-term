...
epochs_general_detector = [150, 300]
epochs_qualified_detectors = [100, 150]
...
if __name__ == '__main__':
    ...
    for name, (train, validation, labels, _) in [('gibson', get_final_doors_dataset_all_envs()), ('deep_doors_2', get_deep_doors_2_relabelled_dataset_for_gd()), ('gibson_deep_doors_2', get_gibson_and_deep_door_2_dataset(half=False))]:
        ...
        model, compute_loss, optimizer, scheduler, scaler, start_epoch, nl, nw, nb, amp, nbs, accumulate, lf, logs = prepare_model(globals()[f'EXP_GENERAL_DETECTOR_{name}_{epochs_general_detector[epoch_count]}_EPOCHS'.upper()], reload_model=False, restart_checkpoint=False, epochs=epochs_general_detector[-1])
        print_logs_every = 10
        last_opt_step = -1
        model.to('cuda')
