...
if __name__ == '__main__':
    ...
    for house, epochs_general, dataset, quantity in [(h, e, d, q) for h in ['floor4_run2'] for e in epochs_general_detector for d in datasets for q in fine_tune_quantity]:
        epochs_fine_tune = [40]
        ...
        model.set_description(globals()[f'EXP_2_{house}_{dataset}_{epochs_general}_FINE_TUNE_{quantity}_EPOCHS_{epochs_fine_tune[0]}_DAY_NIGHT'.upper()])
        print_logs_every = 10
        ...
        for epoch in range(epochs_fine_tune[-1]):
            ...
            if epoch == epochs_fine_tune[epoch_count] -1 and epoch_count < len(epochs_fine_tune) -1:
                epoch_count += 1
                model.set_description(globals()[f'EXP_2_{house}_{dataset}_{epochs_general}_FINE_TUNE_{quantity}_EPOCHS_{epochs_fine_tune[epoch_count]}_DAY_NIGHT'.upper()])
