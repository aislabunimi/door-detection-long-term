...
for num_bboxes in [100]:
    for quantity in [0.75]:
        for house in ['floor1', 'floor4', 'chemistry_floor0', 'house_matteo']:

            save_path = 'results/fine_tune_target_no_sup'
            ...
            bbox_model.set_description(globals()[f'IMAGE_NETWORK_GEOMETRIC_BACKGROUND_GIBSON_DD2_FINE_TUNE_{house}_{int(quantity*100)}_bbox_{num_bboxes}_no_sup'.upper()])
            bbox_model.to('cuda')
