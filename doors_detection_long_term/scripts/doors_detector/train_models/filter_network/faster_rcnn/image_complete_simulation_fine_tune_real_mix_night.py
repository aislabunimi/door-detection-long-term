...
for num_bboxes in [10, 30, 50, 100]:
    for quantity in [0.25, 0.50, 0.75]:
        for house in ['floor1_mix_night', 'floor1_mix_night']:
            ...
            with torch.no_grad():
                for h in [house, house.replace('_mix_night', '')]:
            ...
            bbox_model.set_description(globals()[f'IMAGE_NETWORK_GEOMETRIC_BACKGROUND_GIBSON_DD2_FINE_TUNE_{house}_{int(quantity*100)}_bbox_{num_bboxes}'.upper()])
            bbox_model.to('cuda')
            ...
            logs = {'train': {'loss_label':[], 'loss_confidence':[], 'loss_final':[], 'loss_suppress':[]},
                    ...
                    'test_real_world': {h:{'loss_label':[], 'loss_confidence':[], 'loss_final':[],'loss_suppress':[]} for h in [house, house.replace('_mix_night', '')]},
                    'ap': {0: [], 1: []},
