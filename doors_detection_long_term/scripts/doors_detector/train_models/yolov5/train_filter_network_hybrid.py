...
for env in ['floor1', 'floor4', 'chemistry_floor0', 'house_matteo']:
    if not os.path.exists(os.path.dirname(__file__) + '/filter_network_hybrid/' + env):
        os.mkdir(os.path.dirname(__file__) + '/filter_network_hybrid/' + env)
    save_path = os.path.dirname(__file__) + '/filter_network_hybrid/' + env
    ...
    bbox_model = BboxFilterNetworkGeometricBackground(initial_channels=7, image_grid_dimensions=grid_dim, n_labels=3, model_name=BBOX_FILTER_NETWORK_GEOMETRIC_BACKGROUND, pretrained=False, dataset_name=FINAL_DOORS_DATASET, description=globals()[('IMAGE_NETWORK_GEOMETRIC_BACKGROUND_HYBRID_' + env).upper()],
                                                      description_background=globals()[('IMAGE_GRID_NETWORK_GIBSON_DD2_SMALL_' + env).upper()])
    bbox_model.to('cuda')
    ...
    for n, p in bbox_model.named_parameters():
        if any([x in n for x in ['fpn.bn1.weight', 'fpn.bn1.bias', 'fpn.layer1',]]):
