...
if __name__ == '__main__':
    ...
    for house in houses:
        ...
        model, compute_loss, optimizer, scheduler, scaler, start_epoch, nl, nw, nb, amp, nbs, accumulate, lf, logs = prepare_model(globals()[f'EXP_GENERAL_DETECTOR_HYBRID_{house}_{epochs_general_detector[epoch_count]}_EPOCHS'.upper()], reload_model=False, restart_checkpoint=False, epochs=epochs_general_detector[-1])
        print_logs_every = 10
        last_opt_step = -1
        model.to('cuda')
        ...
        for epoch in range(epochs_general_detector[-1]):
            ...
            model.train()
            optimizer.zero_grad()
            for d, data in tqdm(enumerate(data_loader_train), total=len(data_loader_train), desc=f'Epoch {epoch} - Train GD without {house.upper()}'):
                ni = d + nb * epoch
            ...
            with torch.no_grad():
                accumulate_loss = []
                for i, data in tqdm(enumerate(data_loader_train), total=len(data_loader_train), desc=f'Epoch {epoch} - Test model with training without {house.upper()} dataset'):
                    images, targets, converted_boxes = data
            ...
            with torch.no_grad():
                accumulate_loss = []
                for i, data in tqdm(enumerate(data_loader_validation), total=len(data_loader_validation), desc=f'Epoch {epoch} - Test model with validation data without {house.upper()} dataset'):
                    images, targets, converted_boxes = data
            ...
            if epoch == epochs_general_detector[epoch_count] - 1 and epoch_count < len(epochs_general_detector) -1:
                epoch_count += 1
                model.set_description(globals()[f'EXP_GENERAL_DETECTOR_HYBRID_{house}_{epochs_general_detector[epoch_count]}_EPOCHS'.upper()])
