...
houses = pd.read_excel('./../../results/yolo_v5_results_real_data.xlsx')
...
for dataset in ['deep_doors_2', 'gibson', 'gibson_deep_doors_2']:
    closed_doors = houses[(houses.label == 0) & (houses.general_dataset == dataset)][['house', 'exp', 'AP']]
    closed_doors = closed_doors.pivot_table(values=['AP'], index=closed_doors['house'], columns='exp', aggfunc='first').reset_index()
    closed_doors['house'] = closed_doors['house'].astype(houses_list_dtype)
    closed_doors = closed_doors.sort_values(['house'])

    open_doors = houses[(houses.label == 1) & (houses.general_dataset == dataset)][['house', 'exp', 'AP']]
    open_doors = open_doors.pivot_table(values=['AP'], index=open_doors['house'], columns='exp', aggfunc='first').reset_index()
    ...
    X = np.arange(4)
    ax.bar(X, closed_doors[('AP', experiments[0])].tolist() + [closed_doors[('AP', experiments[0])].mean()], yerr=np.array([[0, 0] for _ in range(3)] + [[closed_doors[('AP', experiments[0])].std(), closed_doors[('AP', experiments[0])].std()]]).T,width=0.2, label=labels[0])
    ax.bar(X + 0.2, closed_doors[('AP', experiments[1])].tolist() + [closed_doors[('AP', experiments[1])].mean()], yerr=np.array([[0, 0] for _ in range(3)] + [[closed_doors[('AP', experiments[1])].std(), closed_doors[('AP', experiments[1])].std()]]).T,  width=0.2, label=labels[1])
    ...
    X = np.arange(4)
    ax.bar(X, open_doors[('AP', experiments[0])].tolist() + [open_doors[('AP', experiments[0])].mean()], yerr=np.array([[0, 0] for _ in range(3)] + [[open_doors[('AP', experiments[0])].std(), open_doors[('AP', experiments[0])].std()]]).T, width=0.2, label=labels[0])
    ax.bar(X + 0.2, open_doors[('AP', experiments[1])].tolist() + [open_doors[('AP', experiments[1])].mean()], yerr=np.array([[0, 0] for _ in range(3)] + [[open_doors[('AP', experiments[1])].std(), open_doors[('AP', experiments[1])].std()]]).T,  width=0.2, label=labels[1])
