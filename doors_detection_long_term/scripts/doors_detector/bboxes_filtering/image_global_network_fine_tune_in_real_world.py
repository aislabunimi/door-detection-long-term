...
from doors_detection_long_term.doors_detector.models.background_grid_network import SharedMLP, TEST_IMAGE_GLOBAL_NETWORK
...
class BboxFilterNetworkGeometric(GenericModel):
    ...
    def forward(self, images, bboxes):
        feature_images = self.resnet(images).unsqueeze(-1)
        ...
        global_feature_images = feature_images.repeat(1, 1, local_features_1.size(-1))
        ...
        mixed_features = torch.cat([local_features_1, global_features_1, global_feature_images], 1)
...
for epoch in range(60):
    ...
    plt.title('AP')
    plt.legend()
    plt.savefig('image_global_net/AP_fine_tuned.svg')
    ...
    plt.title('Complete metric')
    plt.legend()
    plt.savefig('image_global_net/complete_metric_finetuned.svg')
    ...
    plt.legend()
    plt.savefig('image_global_net/train_geometric_fine_tuned.svg')
    ...
    plt.legend()
    plt.savefig('image_global_net/val_geometric_finetuned.svg')
    print(test_accuracy)
    ...
    plt.title('Losses')
    plt.legend()
    plt.savefig('image_global_net/losses_geometric_label_finetuned.svg')
    ...
    plt.title('Losses')
    plt.legend()
    plt.savefig('image_global_net/losses_geometric_confidence_finetuned.svg')
