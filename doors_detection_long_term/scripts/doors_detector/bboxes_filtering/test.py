...
from doors_detection_long_term.doors_detector.models.background_grid_network import BboxFilterNetwork, TEST
...
num_boxes = 15
model = BboxFilterNetwork(BBOX_FILTER_NETWORK, pretrained=False, num_bboxes=num_boxes, dataset_name=FINAL_DOORS_DATASET, description=TEST)
print(model)
...
for images, targets, converted_boxes in data_loader_train:
    o = model(images, torch.ones((params['batch_size'], num_boxes, 7)))
