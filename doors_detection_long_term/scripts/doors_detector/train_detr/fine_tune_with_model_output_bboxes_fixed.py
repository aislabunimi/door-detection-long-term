...
for house in houses.keys():
        ...
        for i, (scores, labels, bboxes) in enumerate(zip(scores_images, labels_images, pred_boxes_images)):
            ...
            if keep_len > 0:
                ...
                if np.count_nonzero(gt_mask) == 0:
                    ...
                    bboxes = gt_bboxes
                    bboxes = np.array([(box._x, box._y, box._x + box._w, box._y + box._h) for box in bboxes]) * [w, h, w, h]
                    labels = [int(box.get_class_id()) for box in gt_bboxes]
    ...
    final = '_train_size_' + str(percentage) + '_confidence_' + str(threshold) + '_model_output_bboxes_fixed'
